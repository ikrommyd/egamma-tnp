...
class Commissioning_hists:
    ...
    def plot_var_histogram(self, samples_config, vars_config, samples, var, pileup_corr, ax, probe_region, color, norm_val=None):
        ...
        if "mass" in var:
            legend += rf" [$\mu$={h_weighted_mean.value:.2f}, $\sigma$={np.sqrt(h_weighted_mean.variance):.2f}]"
    ...
    def plot_fill_between_uncertainty(self, h, ax, color):
        ...
        ax[1].fill_between(h.to_numpy(flow=True)[1][:-1], lower_bound, upper_bound, hatch="XXXXX", step="post", facecolor="none", edgecolor=color, linewidth=0)
        return 0
                ...
                if var in common_fileds:
                    ...
                    for sample in target_samples_:
                        ...
                        hist_target_, yerr_target_, bin_width_ = self.plot_var_histogram(
                            ...
                            probe_region=probe_region,
                            norm_val=norm_val,
                            color=target_samples_color[sample],
                        )
...
if __name__ == "__main__":
    out = Commissioning_hists(fileset_json="config/fileset_2024C_two_versions.json", var_json="config/default_commissionig_binning.json", use_dask=False)
