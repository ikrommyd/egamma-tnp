...
def get_indices_query(input_str: str, maxN: int) -> list[int]:
    ...
    for t in tokens:
        if t.isdigit():
            ...
            rng = t.split("-")
            try:
                for i in range(int(rng[0]), int(rng[1]) + 1):  # including the last index
                    if i > maxN:
                        print(f"[red bold]Requested index {t} larger than available elements {maxN}")
                        return False
...
Some basic commands:
  ...
  - [bold cyan]help[/]: Print this help message
    ...
    def generate_default_metadata(self, dataset):
        ...
        if isMC is True:
            return {"year": year, "isMC": isMC, "xsec": xsec}
        else:
            return {"year": year_data, "isMC": isMC, "primaryDataset": primary_dataset, "era": era_data}
    ...
    def do_select(self, selection=None, metadata=None):
        ...
        if selection is None:
            selection = Prompt.ask("[yellow bold]Select datasets indices[/] (e.g 1 4 6-10)", default="all")
    ...
    def do_replicas(self, mode=None, selection=None):
        ...
        if not indices:
            return
        datasets = [(self.selected_datasets[ind], self.selected_datasets_metadata[ind]) for ind in indices]
    ...
    def do_allowlist_sites(self, sites=None):
        if sites is None:
            sites = Prompt.ask("[yellow]Restrict the available sites to (comma-separated list)").split(",")
    ...
    def do_list_replicas(self):
        ...
        for dataset in datasets:
            if dataset not in self.replica_results:
                print(f"[red bold]No replica info for dataset {dataset}. You need to selected the replicas with [cyan] replicas [/cyan] command[/]")
                return
            tree = Tree(label=f"[bold orange]Replicas for [/][green]{dataset}[/]")
    ...
    def do_save(self, filename=None):
        ...
        if not filename:
            filename = Prompt.ask("[yellow bold]Output file name (.yaml or .json)", default="output.json")
        ...
        for group, datasets in groups.items():
            dataset_info = {"sample": group, "json_output": f"datasets/{group}.json", "files": []}
            for dataset in datasets:
                dataset_info["files"].append({"das_names": [dataset], "metadata": self.selected_datasets_metadata[self.selected_datasets.index(dataset)]})
        ...
        if format == ".yaml":
            with open(filename.replace(".yaml", "_replicas.yaml"), "w") as file:
                yaml.dump(output_definition_withreplicas, file, default_flow_style=False)
        ...
        if Confirm.ask("[red]Do you want to empty your selected samples list?[/]", default=False):
            ...
            self.replica_results_bysite = {}
            self.final_output = None
            print("[green]Selected datasets list emptied![/]")
    ...
    def do_preprocess(
        ...
        if uproot_options is None:
            uproot_options = {}
        if not output_file:
            output_file = Prompt.ask("[yellow bold]Output name", default="output_preprocessing")
    ...
    def load_dataset_definition(
        ...
        for dataset_query, dataset_meta in dataset_definition.items():
            ...
            if query_results_strategy not in ["all", "manual"]:
                raise ValueError("Invalid query-results-strategy option: please choose between: manual|all")
        ...
        if replicas_strategy == "manual":
            ...
            if replicas_strategy not in ["round-robin", "choose", "first"]:
                raise ValueError("Invalid replicas-strategy: please choose between manual|round-robin|choose|first")
...
@click.command()
@click.option("-d", "--dataset-definition", help="Dataset definition file", type=str, required=False)
@click.option(
    "-o", "--output", help="Output name for dataset discovery output (no fileset preprocessing)", type=str, required=False, default="output_dataset.json"
)
@click.option("-fs", "--fileset-output", help="Output name for fileset", type=str, required=False, default="output_fileset")
@click.option("-p", "--preprocess", is_flag=True, default=False, help="Preprocess with dask")
@click.option("--step-size", type=int, default=500000, help="Step size for preprocessing")
@click.option("--scheduler-url", type=str, default=None, help="Dask scheduler URL")
@click.option("-as", "--allow-sites", help="List of sites to be allowlisted", nargs="+", type=str)
@click.option("-bs", "--block-sites", help="List of sites to be blocklisted", nargs="+", type=str)
@click.option("-rs", "--regex-sites", help="Regex string to be used to filter the sites", type=str)
