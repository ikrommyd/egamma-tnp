...
def test_local_compute(do_preprocess, allow_read_errors_with_report, scheduler):
    ...
    assert hpt_pass_barrel.sum(flow=True) + hpt_pass_endcap.sum(flow=True) == 967.0
    assert hpt_all_barrel.sum(flow=True) + hpt_all_endcap.sum(flow=True) == 1073.0
    assert heta_pass.sum(flow=True) == 967.0
    assert heta_all.sum(flow=True) == 1073.0
    assert hphi_pass.sum(flow=True) == 967.0
    assert hphi_all.sum(flow=True) == 1073.0
