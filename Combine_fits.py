...
def create_subplots_for_bin(bin_name, fit_type):
    ...
    if files_found:
        ...
        fig.suptitle(
            f"Bin {bin_number} ({pt_range})\n"
            f"Fit Type: {fit_type.replace('_', ' ').title()}",
            fontsize=18, y=0.98
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Combine Z mass fit plots into 2x2 grid (DATA/MC × Pass/Fail)",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    parser.add_argument(
        "--bin", type=str, required=True,
        choices=available_bins,
        help="Which pt bin to process (e.g., bin00, bin15)"
    )
    parser.add_argument(
        "--type", type=str, required=True,
        choices=available_fit_types,
        help="Which fit type to combine"
    )
    parser.add_argument(
        "--output-dir", type=str, default="combined_plots",
        help="Subdirectory name for output plots (created within bin folder)"
    )
